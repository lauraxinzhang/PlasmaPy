...
eq_distance = 0.1
forces = {
    key: InterParticleForces(key, 100, eq_distance, 1, 1e3, 0.01, 7)
    for key in ("python", "njit", "njit_parallel")
}
...
solutions = {
    engine: simulation.ParticleTracker(forces[engine], x, v, particle).run(
        1e-3 * u.s, dt=1e-6 * u.s
    )
    for engine in ["njit", "njit_parallel"]
}
