...
class MagnetoStatics(abc.ABC):
    ...
    def _magnetic_field(self, p: np.ndarray) -> np.ndarray:
        """
        ...
        """
        warnings.warn(f"Using unoptimized _magnetic_field code in {self}! _magnetic_field for efficient calculation is currently implemented in GeneralWire and CircularWire.")
        return self.magnetic_field(p * u.m).si.value
...
class CircularWire(Wire):
    ...
    def _magnetic_field(p, pt, dl, current, w) -> np.ndarray:
        r"""
        Calculate magnetic field generated by this wire at position `p`
